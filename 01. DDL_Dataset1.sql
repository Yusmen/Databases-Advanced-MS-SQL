...
Id INT PRIMARY KEY IDENTITY NOT NULL,
PassångerId INT NOT NULL,
FlightId INT NOT NULL,
LuggageId INT NOT NULL,
Price DECIMAL(15,2),
CONSTRAINT FK_Tickets_Passengers Foreign Key (PassångerId) References Passengers(Id),
...
INSERT INTO Tickets(Id, PassångerId, FlightId, LuggageId, Price) VALUES 
(1, 69, 17, 3, 247.14),
(2, 87, 12, 47, 447.82),
(3, 91, 48, 59, 404.87),
(4, 18, 76, 69, 276.14),
...
(100, 90, 62, 40, 141.96)
 ...
 where Destination='Ayn Halagim'
  Go
	...
	Select f.Id,SUM(Price) AS  [TotalPrice] From Tickets As t
	Join Flights As f On f.Id=t.FlightId
	GROUP BY f.Id
	order By TotalPrice DESC,f.Id
	--Order them by price (descending), first name (ascending) and last name (ascending).
	Select p.FirstName,p.LastName,Price From Tickets As t
	Join Passengers AS p On p.Id=t.PassångerId
	Order by Price DESC,FirstName,LastName

	Select lt.Type,Count(p.Id) AS [Count] From Luggages As l
	Join Passengers As p ON p.Id=l.PassengerId
	Join LuggageTypes As lt ON lt.Id=l.LuggageTypeId
	GROUP BY lt.Type
	Order by Count DESC,Type
	--ull name (ascending), origin (ascending) and destination (ascending).
	Select FirstName+ ' '+ LastName as [FullName],Origin,Destination  From Tickets As t 
	Join Passengers AS p ON p.Id=t.PassångerId
	Join Flights As f On f.Id=t.FlightId
	order by FullName,Origin,Destination

	Select p.FirstName,p.LastName,p.Age From Tickets AS t
	RiGHT Join Passengers AS p On p.Id=t.PassångerId
	where t.Id IS NULL
	...
	Select p.FirstName,p.LastName,COUNT(f.Id) AS [TotalTrips] From Passengers As p
	Join Tickets As t ON t.PassångerId=p.Id
	JOIN Flights As f ON f.Id=t.FlightId
	GROUP BY FirstName,LastName
	Order By TotalTrips DESC,FirstName,LastName


	Select p.FirstName + ' '+ p.LastName AS [FullName],pl.Name,  f.Origin + ' - ' + f.Destination AS [Trip],lt.[Type] From Passengers As p
	Join Tickets As T ON t.PassångerId=p.Id
	Join Flights As f ON f.Id=t.FlightId
...
Select Destination,COUNT(t.Id) [TripsCount] From Flights As f 
LEFT JOIN Tickets As t On t.FlightId=f.Id
GROUP BY Destination
Order by TripsCount DESC,Destination
...
END
  ...
  DECLARE @trip INT= (Select Id FROM Flights Where Destination=@destination AND Origin=@origin)
  ...
  DECLARE @tripPrice DECIMAL(15,2)= (Select Price FROM Flights AS f
  Join Tickets AS t ON t.FlightId=f.Id
   Where Destination=@destination AND Origin=@origin)

   DECLARE @totalPrice DECIMAL(15,2)=@tripPrice*@peopleCount

   RETURN 'Total price ' + CAST(@totalPrice AS varchar(20))
...
Select Id,Name,Seats,Range From deleted 
  ...
  SET IDENTITY_INSERT DeletedPlanes ON
...
Select k.FirstName,k.FirstName,k.Destination,k.Price From (
...
Where PriceRank=1
Order By Price DESC,k.FirstName,k.LastName
