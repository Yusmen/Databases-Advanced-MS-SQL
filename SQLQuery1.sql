...
GO
CREATE PROC p_Deposit @AccoutId INT,@Amount DECIMAL(15,2)AS
UPDATE Accounts
SET Balance+=@Amount
WHERE Id=@AccoutId
GO

CREATE PROC p_Withdraw  @AccoutId INT,@Amount DECIMAL(15,2)AS
BEGIN
DECLARE @OldBalance DECIMAL(15,2)
SELECT @OldBalance=Balance FROM Accounts WHERE Id=@AccoutId
...
AS
   ...
   SELECT inserted.Id,deleted.Balance,inserted.Balance,GETDATE() FROM inserted JOIN 
