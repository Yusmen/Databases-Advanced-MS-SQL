...
Select e.FirstName,e.LastName,t.Name AS [Towm],a.AddressText From Employees AS e
...
Join Projects AS p ON p.ProjectID=ep.ProjectID AND StartDate>'2002-08-13 00:00:00'
 Order By e.EmployeeID ASC
 GO
  Select TOP 5 e.EmployeeID,e.FirstName,p.Name AS [ProjectName] From Employees As e
Join EmployeesProjects As ep ON ep.EmployeeID=e.EmployeeID
FULL Join Projects AS p ON p.ProjectID=ep.ProjectID AND StartDate<='2005-01-01 00:00:00'
Where e.EmployeeID=24

Go

Select e.EmployeeID,e.FirstName,m.ManagerID,m.FirstName AS [ManagerName] FROM Employees AS e 
 Join Employees AS m ON m.EmployeeID=e.ManagerID 
...
Order By e.EmployeeID ASC
 ...
 Select FirstName,LastName,Salary,dbo.ufn_GetSalaryLevel(Salary) AS [Salary level] From Employees
 GO



 Create Proc udp_EmployeesSalaryAbove
 AS
 ...
 ALTER PROC udp_EmployeesSalaryAbove
 AS 
  Select FirstName,LastName From Employees
 Where Salary>35000
 ORDER BY FirstName
 ...
 Create Proc usp_GetTownsStartingWith(@sequence VARCHAR(10))
 AS
 Select Name AS [Town] From Towns
 Where SUBSTRING(Name,1,1)=@sequence
 GO

 Alter Proc usp_GetTownsStartingWith(@sequence VARCHAR(10))
 AS
 Select Name AS [Town] From Towns
 Where SUBSTRING(Name,1,LEN(@sequence))=@sequence
 Go
...
Select Count(EmployeeId) from EmployeesProjects Where EmployeeID=1
...
DECLARE @count INT=(Select Count(EmployeeID) from EmployeesProjects Where EmployeeID=@emloyeeId)


 IF(@employee IS NULL OR @project IS NULL)
 BEGIN
 ROLLBACK
 RAISERROR('Invalid Id!!',16,2)
 Return
 END
 IF(@count>3)
 BEGIN
 ...
 As
 INSERT INTO DeletedEmployees(EmployeeID,FirstName,LastName,MiddleName,JobTitle,DepartmentID,Salary)
 Select EmployeeID,FirstName,LastName,MiddleName,JobTitle,DepartmentID,Salary From deleted


 Select Top (5) FirstName, LastName From Employees

 Select TOP (2) FirstName + ' ' + LastName As [FullName] From Employees Where Salary>10000
 Order By FullName 
